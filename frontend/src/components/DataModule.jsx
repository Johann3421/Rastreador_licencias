      ...
      return (
        <>
          {permissions.includes('activate') && (
            <button type="button" className="primary-action-button" onClick={() => setActivationRow(row)}>
              Activar ahora
            </button>
          )}
          {canRead && (
          ...
          {canUpdate && (
            <button type="button" className="secondary-button" onClick={() => openEditForm(row)}>
              Editar
            </button>
          )}
        </>
      ...
      return (
        <>
          ...
          {permissions.includes('reserve') && (
            <button type="button" className="secondary-button" onClick={() => licenseAction(row.id, 'release-reservation')}>
              Liberar
            </button>
          )}
        </>
        ...
        <div className="header-actions">
          ...
          {entityForm && canCreate && moduleId !== 'licenses' && (
            <button type="button" onClick={openCreateForm}>
              + Crear {moduleId === 'products' ? 'producto' : moduleId === 'variants' ? 'variante' : moduleId === 'batches' ? 'lote' : moduleId === 'customers' ? 'cliente' : moduleId === 'providers' ? 'proveedor' : moduleId === 'users' ? 'usuario' : 'nuevo'}
            </button>
          )}
          {canCreateLicense && (
            <button
